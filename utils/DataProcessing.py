...
class DatasetProcessingCIFAR_10(Dataset):
    def __init__(self, data_path, img_filename, label_filename, transform=None):
        ...
        labels = np.loadtxt(label_filepath, dtype=np.int64)#[int(x.strip()) for x in fp_label]
        fp_label.close()
        self.label = labels
    ...
    def __getitem__(self, index):
        ...
        label = torch.from_numpy(self.label[index])#torch.LongTensor([self.label[index]])
